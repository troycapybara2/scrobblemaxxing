...
def render_more_stats(full_top_artists, baseline_data):
...
def build_galaxy_data(full_top_artists, meta, recent_counts):
    ...
    if G.number_of_nodes() == 1:
        ...
        pos = nx.spring_layout(G, seed=7, k=1.6, iterations=300, weight="weight")
    ...
    centers = {t: (_norm(x, minx, maxx), _norm(y, miny, maxy)) for t, (x, y) in centers.items()}
...
function glow(color,r){
  ...
  grd.addColorStop(0,color); grd.addColorStop(.25,color);
...
function getNeb(c){
  ...
  const g=s.getContext('2d'); const grd=g.createRadialGradient(r,r,0,r,r,r);
  grd.addColorStop(0,c); grd.addColorStop(.35,c); grd.addColorStop(1,'rgba(0,0,0,0)');
  g.globalAlpha=.5; g.fillStyle=grd; g.beginPath(); g.arc(r,r,r,0,7); g.fill();
...
function TX(x){return PAD+(x+1)/2*(W-2*PAD);}
function TY(y){return PAD+(y+1)/2*(H-2*PAD);}
...
function frame(ts){
  ...
  for(const tr of DATA.territories){
    ...
    const R=(60+tr.weight*230);
    ctx.globalAlpha=.16+tr.weight*0.28;
  ...
  for(const tr of DATA.territories){
    ...
    ctx.fillStyle=hexA(tr.color,.9); ctx.shadowColor=tr.color; ctx.shadowBlur=12;
  ...
  for(const s of DATA.stars){
    ...
    const r=(7+s.size*16)*(1+s.recent*0.5);
    ctx.globalAlpha=s.bright*tw*(0.5+s.recent*0.4);
  ...
  for(const s of DATA.stars){
    ...
    const by=TY(s.y)+Math.cos(t*0.4+s.phase)*1.6;
    const tw=0.72+0.28*Math.sin(t*2.0+s.phase);
    ctx.globalAlpha=Math.min(1,s.bright*tw+0.15);
    ctx.fillStyle='#ffffff';
    ctx.beginPath(); ctx.arc(bx,by,1.1+s.size*1.7,0,7); ctx.fill();
  }
  ctx.restore();
...
def main():
    ...
    if view_mode == "More Stats":
        baseline_data = load_long_term_baseline()
        render_more_stats(full_top_artists, baseline_data)
        return
